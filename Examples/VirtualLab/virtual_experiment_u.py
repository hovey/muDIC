...
image_deformer = vlab.imageDeformer_from_uFunc(
    ...
    omega=2 * np.pi / (500.0 * downsample_factor),
...
u_x, u_y = displacement_function(xs, ys, omega=2.0 * np.pi / 500.0, amp=2.0)
...
if show_results:
    ...
    line3 = ax2.plot(
        res_field[:, 25] - fields.disp()[0, 0, :, 25, 1], "r--", label="difference"
    )
